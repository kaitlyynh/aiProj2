...
def row_grid_sub_degs(value, deg):
    print(f"in row_grid_sub_degs value: {value}  deg[0]: {deg[0]}")
    if value == 0:  # Unassigned cells contribute to the degree
        deg[0] += 1
        print("in row_grid_sub_degs incremented", value, deg[0])

def dot_check_degs(dot, cell, deg):
    print(f"in dot_check_degs dot : {dot} cell : {cell} and deg[0] = {deg[0]}")
    if dot != 0 and cell == 0:  # Unassigned cells with dot constraints
        print("in dot_check_degs incremented")
        deg[0] += 1


def degree_heuristic(grid, mrv_results, hor_dots, vert_dots):
    print("IN DH", mrv_results)
    max_degree = -1
    ...
    for coord in mrv_results:
        ...
        print("before row_col_subgrid_traverse", degree[0], "coord is (", r, c, ")")
        ...
        print("degree after row_col_subgrid_traverse", degree[0])
        ...
        print("degree after dot_traverse", degree[0])
    ...
    print("coord chosen: ", max_var, "degree is ", degree[0])
    return max_var
