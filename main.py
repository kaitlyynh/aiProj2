...
def row_col_subgrid_traverse(grid, row, col, action):
    ...
    for c in range(9):
        if c == col: continue
        action(grid[row][c], None, None)

    for r in range(9):
        if r == row: continue
        action(grid[r][col], None, None)
    ...
    for i in range(3):
        for j in range(3):
            if i + startRow == row and j + startCol == col:
                continue
            action(grid[i + startRow][j + startCol], None, None)
...
def dot_traverse(grid, row, col, hor_dots, vert_dots, action):
    ...
    """
    # case 1: dot between me and left (col - 1)
    if 0 <= row < len(hor_dots) and 0 <= col - 1 < len(hor_dots[0]):
        action(hor_dots[row][col - 1], grid[row][col - 1], None)

    # case 2: dot between me and right (col)
    if 0 <= row < len(hor_dots) and 0 <= col < len(hor_dots[0]):
        action(hor_dots[row][col], grid[row][col + 1], None)

    # case 3: dot between me and above (row - 1)
    if 0 <= row - 1 < len(vert_dots) and 0 <= col < len(vert_dots[0]):
        action(vert_dots[row - 1][col], grid[row - 1][col], None)

    # case 4: dot between me and below (row)
    if 0 <= row < len(vert_dots) and 0 <= col < len(vert_dots[0]):
        action(vert_dots[row][col], grid[row + 1][col], None)
...
def isSafe(grid, row, col, num, hor_dots, vert_dots):
    ...
    def diff_check(value, dummy1=None, dummy2=None):
        nonlocal is_diff
        if value == num:
            is_diff = False
    ...
    def dot_check(dot, cell, dummy=None):
        nonlocal is_diff
        if dot == 1 and \
...
def row_grid_sub_degs(value, deg, dummy=None):
...
def degree_heuristic(grid, mrv_results, hor_dots, vert_dots):
    ...
    for coord in mrv_results:
        ...
        row_col_subgrid_traverse(grid, r, c, lambda value, _, __: row_grid_sub_degs(value, degree))

        # Count degree based on the dot constraints
        dot_traverse(grid, r, c, hor_dots, vert_dots, lambda dot, cell, _: dot_check_degs(dot, cell, degree))
